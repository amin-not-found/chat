...
func (mq *MessageQuery) QueryReceivers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, message.ReceiversTable, message.ReceiversPrimaryKey...),
		)
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withReceivers: mq.withReceivers.Clone(),
...
func (mq *MessageQuery) WithReceivers(opts ...func(*UserQuery)) *MessageQuery {
	query := &UserQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withReceivers = query
	return mq
}
...
func (mq *MessageQuery) sqlAll(ctx context.Context) ([]*Message, error) {
	...
	if query := mq.withReceivers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Message, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Receivers = []*User{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(message.ReceiversPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, mq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "receivers": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "receivers" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Receivers = append(nodes[i].Edges.Receivers, n)
			}
		}
	}
	...
	if query := mq.withSender; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_undelivered_messages == nil {
				continue
			}
			fk := *nodes[i].user_undelivered_messages
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_undelivered_messages" returned %v`, n.ID)
			}
			for i := range nodes {
