...
type UserQuery struct {
	...
	// eager-loading edges.
	withUnrecivedMessages   *MessageQuery
	withUndeliveredMessages *MessageQuery
...
// QueryUnrecivedMessages chains the current query on the "unrecived_messages" edge.
func (uq *UserQuery) QueryUnrecivedMessages() *MessageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, user.UnrecivedMessagesTable, user.UnrecivedMessagesPrimaryKey...),
		)
...
// QueryUndeliveredMessages chains the current query on the "undelivered_messages" edge.
func (uq *UserQuery) QueryUndeliveredMessages() *MessageQuery {
	query := &MessageQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(message.Table, message.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.UndeliveredMessagesTable, user.UndeliveredMessagesColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUnrecivedMessages:   uq.withUnrecivedMessages.Clone(),
		withUndeliveredMessages: uq.withUndeliveredMessages.Clone(),
...
// WithUnrecivedMessages tells the query-builder to eager-load the nodes that are connected to
// the "unrecived_messages" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUnrecivedMessages(opts ...func(*MessageQuery)) *UserQuery {
	query := &MessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUnrecivedMessages = query
	return uq
}

// WithUndeliveredMessages tells the query-builder to eager-load the nodes that are connected to
// the "undelivered_messages" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUndeliveredMessages(opts ...func(*MessageQuery)) *UserQuery {
	query := &MessageQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUndeliveredMessages = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withUnrecivedMessages != nil,
			uq.withUndeliveredMessages != nil,
		}
	)
	...
	if query := uq.withUnrecivedMessages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.UnrecivedMessages = []*Message{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.UnrecivedMessagesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "unrecived_messages": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "unrecived_messages" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.UnrecivedMessages = append(nodes[i].Edges.UnrecivedMessages, n)
			}
		}
	}

	if query := uq.withUndeliveredMessages; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UndeliveredMessages = []*Message{}
		}
		query.withFKs = true
		query.Where(predicate.Message(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UndeliveredMessagesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_undelivered_messages
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_undelivered_messages" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_undelivered_messages" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UndeliveredMessages = append(node.Edges.UndeliveredMessages, n)
		}
	}
