...
func (mc *MessageCreate) SetLocalID(i int64) *MessageCreate {
	mc.mutation.SetLocalID(i)
	return mc
}

// SetCanBeDeleted sets the "can_be_deleted" field.
func (mc *MessageCreate) SetCanBeDeleted(b bool) *MessageCreate {
	mc.mutation.SetCanBeDeleted(b)
	return mc
}

// AddReceiverIDs adds the "receivers" edge to the User entity by IDs.
func (mc *MessageCreate) AddReceiverIDs(ids ...int) *MessageCreate {
	mc.mutation.AddReceiverIDs(ids...)
	return mc
}

// AddReceivers adds the "receivers" edges to the User entity.
func (mc *MessageCreate) AddReceivers(u ...*User) *MessageCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return mc.AddReceiverIDs(ids...)
}
...
func (mc *MessageCreate) check() error {
	...
	if _, ok := mc.mutation.LocalID(); !ok {
		return &ValidationError{Name: "local_id", err: errors.New(`ent: missing required field "local_id"`)}
	}
	if _, ok := mc.mutation.CanBeDeleted(); !ok {
		return &ValidationError{Name: "can_be_deleted", err: errors.New(`ent: missing required field "can_be_deleted"`)}
	}
	return nil
}
...
func (mc *MessageCreate) createSpec() (*Message, *sqlgraph.CreateSpec) {
	...
	if value, ok := mc.mutation.CanBeDeleted(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeBool,
			Value:  value,
			Column: message.FieldCanBeDeleted,
		})
		_node.CanBeDeleted = value
	}
	if nodes := mc.mutation.ReceiversIDs(); len(nodes) > 0 {
	...
	if nodes := mc.mutation.SenderIDs(); len(nodes) > 0 {
		...
		_node.user_undelivered_messages = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}
